...
const Test = () => {
    let { data, isSuccess } = useConnectorClient()
    const { encryptionPublicKey, fetch, fetchStatus, isFetched } = useEncryptionPublicKey(
        isSuccess && data ? data.account.address : ZeroAddress
    )
    ...
    return (
        <>
            <VStack>
                ...
                <Button onClick={() => fetch()}>Fetch Public Encryption Key</Button>
                <SendMessageForm />
